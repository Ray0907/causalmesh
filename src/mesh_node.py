...
class MeshNode:
    ...
    def _update_chain(self):
        ...
        sorted_node_ids = sorted(list(self.peers.keys()) + [self.node_id])
        my_index = sorted_node_ids.index(self.node_id)
        successor_index = (my_index + 1) % len(sorted_node_ids)
        successor_id = sorted_node_ids[successor_index]

        if successor_id == self.node_id:
            self.successor = None # Running solo
        else:
            self.successor = self.peers[successor_id]
    ...
    async def _handle_join_request(self, message: Dict[str, Any]) -> Dict[str, Any]:
        ...
        all_nodes_in_mesh.append(PeerInfo(self.node_id, self.host, self.port).__dict__)
        ...
        # Add the new peer to this node's list *after* broadcasting to others
    ...
    async def _handle_server_write(self, message: Dict[str, Any]) -> None:
        ...
        sorted_node_ids = sorted(list(self.peers.keys()))
        origin_index = sorted_node_ids.index(origin_node)
        tail_index = (origin_index - 1 + len(sorted_node_ids)) % len(sorted_node_ids)
        tail_id = sorted_node_ids[tail_index]
        ...
        if is_tail and current_round == 2:
            ...
            self.logger.info(f"Write for key '{key}' (vc={vc}) reached tail. Integrating.")
    ...
    def _dependency_integration(self, deps: Dict[str, VectorClock]):
        ...
        for key, vc in all_deps_to_integrate.items():
            ...
            if version_to_integrate:
                ...
                self.logger.debug(f"Integrated key '{key}' (vc={version_vc}) into C-cache.")
